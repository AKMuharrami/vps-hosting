...
const FONT_URLS: Record<string, string> = {
  "Janna LT":
    "https://hjrm8lbtnby37npy.public.blob.vercel-storage.com/Janna%20LT%20Regular.ttf",
  Cairo:
  ...
  "IBM Plex Sans Arabic":
    "https://raw.githubusercontent.com/google/fonts/main/ofl/ibmplexsansarabic/IBMPlexSansArabic-Bold.ttf",
  "DejaVu Sans":
...
async function ensureFont(fontName: string): Promise<string | null> {
  ...
  const normalizedKey = Object.keys(FONT_URLS).find(
    (k) => k.toLowerCase() === fontName.toLowerCase(),
  );
...
const exportJobs = new Map<
  string,
  { status: string; progress?: number; downloadUrl?: string; error?: string }
>();
...
app.get("/api/download-export/:fileId", async (req, res) => {
  ...
  if (!fs.existsSync(filePath)) {
    return res
      .status(404)
      .send("File not found or expired. Please export again.");
  }
...
app.post(
  ...
  async (req: any, res: any) => {
    ...
    const safeOriginalName = (originalName || "video.mp4").replace(
      /[^a-zA-Z0-9._-]/g,
      "_",
    );
    ...
    const GPU_BACKEND_URL =
      process.env.VAST_GPU_URL || process.env.GPU_BACKEND_URL;
    if (GPU_BACKEND_URL) {
      console.log(
        `[Export Pipeline] Delegating rendering to GPU Backend at: ${GPU_BACKEND_URL}`,
      );
      (async () => {
        try {
          ...
          if (req.body.captionsJson)
            formData.append("captionsJson", req.body.captionsJson);
          if (req.body.styleOptions)
            formData.append("styleOptions", req.body.styleOptions);
          ...
          if (!response.ok) {
            ...
            throw new Error(
              `GPU Backend failed with status ${response.status}: ${errBody}`,
            );
          }

          const gpuResponse = (await response.json()) as { jobId: string };
          console.log(
            `[Export Pipeline] GPU task accepted. Remote Job ID: ${gpuResponse.jobId}`,
          );
          ...
          const pollInterval = setInterval(async () => {
            try {
              const statRes = await fetch(
                `${GPU_BACKEND_URL}/api/export-status/${remoteJobId}`,
              );
          ...
          console.error(
            "[Export Pipeline] Proxy to GPU Backend Failed:",
            error,
          );
          exportJobs.set(sessionId, {
            status: "failed",
            error: "GPU Backend communication failed.",
          });
        } finally {
    ...
    (async () => {
      ...
      try {
        if (videoSource.startsWith("http")) {
          console.log(
            `[Export Background] Downloading video from URL to avoid FFmpeg TLS issues...`,
          );
        ...
        try {
          if (req.body.captionsJson)
            captionsJson =
              typeof req.body.captionsJson === "string"
                ? JSON.parse(req.body.captionsJson)
                : req.body.captionsJson;
          if (req.body.styleOptions)
            styleOptionsParsed =
              typeof req.body.styleOptions === "string"
                ? JSON.parse(req.body.styleOptions)
                : req.body.styleOptions;
        } catch (e) {}
        ...
        if (captionsJson && styleOptionsParsed) {
          ...
          const possiblePaths = [
            ...
            "/root/vps-hosting/remotion/index.ts",
            "/root/vps-hosting/standalone-backend/remotion/index.ts",
          ...
          if (!remotionEntryPoint) {
            console.error("[Export] Critical: Cannot find remotion/index.ts.");
            console.error("Checked these paths: ", possiblePaths);
            console.error("Current __dirname:", __dirname);
            console.error("Current cwd:", process.cwd());
            throw new Error(
              "Cannot find remotion directory. Please export/push your latest code to GitHub, OR execute on Vast: cp -r ../remotion ./remotion ",
            );
          }
          ...
          console.log(
            `[Export] Final Render Config: w=${targetW}, h=${targetH}, frames=${durationInFrames}`,
          );
          ...
          try {
            ...
            console.log(
              "[Export] Remotion rendering completed (video only). Muxing audio...",
            );
            ...
            await new Promise((resolve, reject) => {
              ffmpeg()
                ...
                .on("error", (err) => {
                  console.error(
                    "[Export] Muxing error, falling back to video only:",
                    err,
                  );
                  try {
            ...
            throw new Error(
              `Remotion Engine Error: ${renderErr.message || renderErr}`,
            );
          }
        } else {
          ...
          const escapedSrtPath = srtFileName
            .replace(/\\/g, "/")
            .replace(/'/g, "'\\''");
          ...
          console.log(
            `[Export Background] Starting FFmpeg Filter: ${filterStr}`,
          );
          ...
          await new Promise<void>((resolve, reject) => {
            ...
            ffmpegProcess.on("close", (code, signal) => {
              if (code === 0) resolve();
                ...
                reject(
                  new Error(
                    `FFmpeg exited with code ${code}, signal: ${signal}, stderr: ${errorOutput}`,
                  ),
                );
            });
